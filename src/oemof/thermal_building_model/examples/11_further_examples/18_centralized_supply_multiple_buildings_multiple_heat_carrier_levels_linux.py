"""Run the centralized district-level heat-supply optimization workflow.

This executable builds combined SFH/MFH district cases for selected UEU
clusters, heat-grid supply temperatures, refurbishment scenarios, and k-values.
It uses processed cluster input data, weather data, heat-grid lengths, and
economic configuration files to run centralized heat-supply investment
optimizations with CO2 and electricity-peak reduction sweeps. Outputs are
scenario definitions and pickle result files for centralized district-level
heat-supply cases. This is the main centralized workflow entry point used to
reproduce the district heat-supply analyses in the manuscript.
"""
...
from oemof.thermal_building_model.oemof_facades.technologies.storages import HotWaterTank, SeasonalWaterTank
...
from oemof.thermal_building_model.helpers.define_building_combinations_for_cen_optimization import remove_duplicate_scenarios, build_scenarios
...
from oemof.thermal_building_model.helpers.co2_parallel import run_co2_peak_sweep_to_files
from oemof.thermal_building_model.helpers.optimization_io import (
    atomic_pickle_dump as _atomic_pickle_dump,
    co2_factor_to_suffix as _co2_factor_to_suffix,
    is_reference_k as _is_reference_k,
    k_to_folder_token as _format_k_for_folder,
    normalize_result_root,
    parse_csv_tokens as _parse_csv_tokens,
    parse_k_values as _parse_k_values,
    script_base_path,
)
from oemof.thermal_building_model.helpers.cluster_io import (
    discover_available_k_values as _discover_available_k_values,
    load_cluster_for_type,
)
...
from oemof.thermal_building_model.helpers.process_cluster import (
    process_cluster_centralized as process_cluster,
)
...
# Build and solve one centralized district-level heat-supply investment model.
def run_model(co2_new,peak_new,data,aggregation1,t1_agg,data_classes_comp,combined_cluster,heat_grid_temperature,cluster_occurence,heat_demand_worst_case,heat_grid_length):
    ...
    # Central heat carrier for the heat-grid supply side.
    ...
    # Central hot-water thermal storage.
    ...
    # Central seasonal thermal storage.
    ...
    def configure_central_battery_capacity(battery_config_building, key):
    ...
    add_batteries(
        ...
        configure_config=configure_central_battery_capacity,
    )
    ...
    print(demand)
    # Heat-grid investment component.
    ...
    # Building-side heat demand, electricity demand, and PV connected to the heat grid.
    for index, row in combined_cluster.iterrows():
        ...
        components[building_id]={}
        buildings_in_cluster =row['buildings_in_cluster']
        # Building heat demand behind the heat grid.
        ...
        # Building electricity carrier and electricity demand.
        ...
        add_pv_systems(
            ...
            reference_unit_quantity=buildings_in_cluster,
        )
    ...
    try:
        ...
        # Heat-grid and building-level component postprocessing.
        for building_id, building_data in components.items():
...
def _load_cluster_for_type(base_path, cluster_name, building_type, k_value):
    return load_cluster_for_type(
        base_path,
        cluster_name,
        building_type,
        k_value,
        add_reference_buildings_in_cluster=True,
    )
...
def _normalize_result_root(raw_value, base_path):
    return normalize_result_root(raw_value, base_path, label="storage")
...
def run_main(heat_grid_temperature,ueu,heat_grid_length,sfh_k_value,mfh_k_value,scenario_mode="all"):
    ...
    # Load selected SFH/MFH cluster representatives for the district case.
    ...
    # Prepare weather and building input data used for scenario construction.
    ...
    # Store the refurbishment scenario definitions used by the optimization run.
    ...
    for scenario in scenarios:
        # Rebuild model input tables for the current district refurbishment scenario.
        location = calculate_gain_by_sun.Location(
            epwfile_path=os.path.join(
                main_path,
                "thermal_building_model",
        ...
        # Aggregate annual profiles to representative periods for the MILP.
        ...
        if False:
            for index, row in combined_cluster.iterrows():
                try:
                    ...
                    # Identify the demand columns for each building_id.
                    ...
                    # Scale demand columns by the number of buildings in the cluster.
        ...
        for ref in references:
            if ref=="co2":
                # Store one full and one reduced result pickle per CO2 reduction step.
                ...
                def run_centralized_case(co2_new, peak_new):
                    return run_model(
                        co2_new,
                        peak_new,
                        data,
                ...
                run_co2_peak_sweep_to_files(
                    run_case=run_centralized_case,
...
ERROR_DIR = "../04_applied_energy_optimization_post_process/error_logs"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run centralized district heat-supply optimization cases with optional host job slicing."
    )
    parser.add_argument("--host-name", type=str, default="unknown", help="Optional host label used in log and error output.")
    parser.add_argument("--job-start", type=int, default=0, help="Start index in the generated global job list for this host.")
    parser.add_argument("--max-jobs", type=int, default=None, help="Maximum number of centralized jobs assigned to this host.")
    parser.add_argument("--workers", type=int, default=None, help="Number of parallel worker processes on this host.")
    parser.add_argument("--serial", action="store_true", help="Run selected centralized jobs sequentially.")
    ...
    parser.add_argument(
        ...
        choices=["all", "capex_min_only"],
        help="Refurbishment scenario subset to run: all generated scenarios or only the capex-minimum scenario.",
    )
    parser.add_argument(
        "--temps",
        type=str,
        default=",".join(str(x) for x in DEFAULT_HEAT_GRID_SUPPLY_TEMPERATURES),
        help="Comma-separated heat-grid supply temperatures in degC, e.g. 50,60,80.",
    )
    parser.add_argument(
        "--sfh-k",
        type=str,
        default=",".join(str(x) for x in DEFAULT_K_VALUES_TO_OPTIMIZE_SFH),
        help="Comma-separated SFH cluster k-values used in district combinations, e.g. reference,1,2,4.",
    )
    parser.add_argument(
        "--mfh-k",
        type=str,
        default=",".join(str(x) for x in DEFAULT_K_VALUES_TO_OPTIMIZE_MFH),
        help="Comma-separated MFH cluster k-values used in district combinations, e.g. reference,1,2,3.",
    )
    parser.add_argument(
        "--ueu-cases",
        type=str,
        default=",".join(f"{name}:{length}" for name, length in DEFAULT_UEU_CASES),
        help="Comma-separated UEU cases as <ueu>:<heat_grid_length_in_m>.",
    )
    parser.add_argument(
        "--result-storage-root",
        type=str,
        default="default",
        help=(
            "Base directory for centralized scenario and result pickle files. "
            "Use 'default' to store results near this script."
        ),
    )
